...
class QuestionClassifier:
    def __init__(self):
        ...
        self.knowledge_representation_wds = [i.strip() for i in
                                             open(self.knowledge_representation_path, encoding='utf-8') if i.strip()]
        self.knowledge_extract_wds = [i.strip() for i in open(self.knowledge_extract_path, encoding='utf-8') if
                                      i.strip()]
        self.knowledge_storage_wds = [i.strip() for i in open(self.knowledge_storage_path, encoding='utf-8') if
                                      i.strip()]
        self.knowledge_engineer_wds = [i.strip() for i in open(self.knowledge_engineer_path, encoding='utf-8') if
                                       i.strip()]
        self.knowledge_incorporate_wds = [i.strip() for i in open(self.knowledge_incorporate_path, encoding='utf-8') if
                                          i.strip()]
        self.knowledge_graph_wds = [i.strip() for i in open(self.knowledge_graph_path, encoding='utf-8') if i.strip()]
        self.knowledge_graph_project_wds = [i.strip() for i in open(self.knowledge_graph_project_path, encoding='utf-8')
                                            if i.strip()]
        ...
        self.region_words = set(
            self.semantic_search_wds + self.knowledge_representation_wds + self.knowledge_extract_wds +
        ...
        # 问句疑问词
        self.contain_qwds = ['包含', '包括', '涉及', '涵盖', '其中一个', '有哪些方面', '有几个', '含有', '子概念',
                             '有几类', '类别']
        self.equal_qwds = ['等于', '相当于', '相同', '一致', '相同的意思', '等价', '同义', '一回事', '对等', '等同',
                           '同一个', '又称', '别名']
        self.derive_qwds = ['来自', '体现', '基础', '源于', '起源', '产生', '衍生', '根源', '由来', '来源']
        self.belong_qwds = ['归类', '属于', '所属', '从属', '隶属于', '归属']
        self.realize_qwds = ['方法', '采用', '使用', '通过', '用', '基于', '解决', '核心', '关键', '设计', '构建',
                             '实施', '实行', '优化'
        ...
        self.define_qwds = ['定义', '是什么意思', '是什么东西', '是指什么', '什么是', '意思', '含义', '是啥意思',
                            '是啥东西', '是指啥', '啥是', '即什么',
                            '解释', '啥叫', '内容', '详细信息', '描述', '是什么']
        self.engname_qwds = ['英文名', '用英语怎么说', '译名', '英文缩写', '英文简称', '英文称谓', '英文名称']
        self.target_qwds = ['目标', '目的', '意图', '预期', '旨在', '致力于', '是为了']
        self.content_qwds = ['定义', '是什么意思', '是什么东西', '是指什么', '什么是', '意思', '含义', '是啥意思',
                             '是啥东西', '是指啥', '啥是', '即什么',
                             '解释', '啥叫', '内容', '详细信息', '描述', '是什么']
        self.effect_qwds = ['影响', '作用', '能做什么', '有什么用', '有啥用', '功能', '用途', '效果', '可以干什么',
                            '是干什么的', '干啥']
        self.character_qwds = ['特点', '针对', '解决了', '特色', '支持', '优点', '特征', '优势', '独特', '强项', '性质',
                               '长处', '好处']
        self.method_qwds = ['方法', '采用', '使用', '通过', '用', '基于', '解决', '核心', '关键', '设计', '构建',
                            '实施', '实行', '优化'
            , '部署', '开发', '验证', '探索', '整合', '调研', '调查', '规划', '迭代', '策略',
                            '完成', '达成', '实现', '实行', '用了什么技术', '技术要点', '手段']
        self.flaw_qwds = ['问题', '缺点', '但需要', '需要花费', '无法', '太弱', '缺少', '低下', '耗费', '差', '不高',
                          '耗时', '费力', '代价',
                          '弊端', '局限', '难题', '不足', '限制', '风险', '缺乏', '效率低', '不可靠', '复杂', '易错',
                          '成本高', '不适用', '依赖性', '开销']
        self.create_time_qwds = ['年', '月', '时间', '日期', '创建时间', '几号', '天', '时候', '多久', '好久', '天数']
        self.creator_qwds = ['专家', '大学', '实验室', '公司', '创始人', '研究院', '研究所', '基金会', '工程师', '学者',
                             '学家', '等人', '提出', '研究计划局',
                             '会议', '组织', '发起', '领导', '建立', '创建', '研究者', '带领', '创建', '创办', '成立了',
                             '导师', '谁']
